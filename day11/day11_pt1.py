...
class Universe:
    ...
    def expanding_taxicab_distance(self, galaxy1, galaxy2):
        ...
        if y1 > y2:
            row_expansions = sum([1 for row in rows if row in range(y2, y1)])
        else:
            row_expansions = sum([1 for row in rows if row in range(y1, y2)])
        
        if x1 > x2:
            column_expansions = sum([1 for column in columns if column 
                                     in range(x2, x1)])
        else:
            column_expansions = sum([1 for column in columns if column 
                                     in range(x1, x2)])
